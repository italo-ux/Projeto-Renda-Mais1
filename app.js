...
// CORREÇÃO: Adicionado req.session.save para garantir a persistência do cookie
...
        return res.status(500).json({ erro: "Erro de sessão no servidor" });
      }
...
// ROTA GET DA PRIMEIRA VISITA (Verifica se Meta ou Renda são NULL)
...
// CORREÇÃO: ROTA POST DA PRIMEIRA VISITA (Única e com req.session.save)
...
    
    // Atualiza a sessão para que a próxima requisição GET /api/usuario tenha os dados
    req.session.usuario.metaMensal = metaMensal;
    req.session.usuario.rendaMensal = rendaMensal;

    req.session.save(err => {
      if (err) {
        console.error("Erro ao salvar sessão após primeira visita:", err);
        return res.status(500).json({ erro: "Erro de sessão no servidor" });
      }
