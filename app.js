...
app.post("/api/registro", async (req, res) => {
  ...
  try {
    const [existe] = await pooldb.query(
...
app.post("/api/atualizar-config", autenticar, async (req, res) => {
  ...
  if (rendaMensal === undefined || dinheiroGuardado === undefined)
    return res
      .status(400)
      .json({ erro: "Campos rendaMensal e dinheiroGuardado são obrigatórios." });
  ...
  try {
    ...
    if (rows.length > 0) req.session.usuario = rows[0];

    res.json({
      mensagem: "Configurações de renda e dinheiro guardado atualizadas com sucesso",
    });
  } catch (err) {
...
if (!PORT) {
  console.error(
    "❌ Nenhuma porta recebida via process.env.PORT (Railway exige isso)."
  );
  process.exit(1);
}
